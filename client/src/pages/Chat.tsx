...
export default function Chat() {
  ...
  return (
    <div className="flex h-screen bg-background">
      {/* Sidebar */}
      <div
        className={`${sidebarOpen ? "w-72" : "w-16"} border-r border-border bg-sidebar transition-all duration-300 flex flex-col`}
      >
        {/* Sidebar Header */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-2xl w-full">
                {SUGGESTED_PROMPTS.map((prompt, i) => (
                  <button
                    ...
                    className="text-left px-4 py-3 rounded-xl border border-border hover:border-primary/30 hover:bg-accent/30 transition-all duration-200 text-sm text-muted-foreground hover:text-foreground active:scale-[0.98]"
                  >
                    {prompt}
                  </button>
                ))}
            ...
            <ScrollArea className="h-full">
              <div className="flex flex-col space-y-4 p-4 max-w-4xl mx-auto">
                {messages.map((msg, idx) => (
                  ...
                  >
                    {msg.role === "assistant" && (
                      <div className="w-8 h-8 shrink-0 rounded-xl bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center mt-1 shadow-sm">
                    ...
                    {msg.role === "user" && (
                      <div className="w-8 h-8 shrink-0 rounded-xl bg-secondary flex items-center justify-center mt-1">
                ...
                {isLoading && (
                  <div className="flex gap-3 justify-start animate-in fade-in duration-200">
                    <div className="w-8 h-8 shrink-0 rounded-xl bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center mt-1">
        ...
        {isDragOver && (
          <div className="absolute inset-0 bg-primary/5 border-2 border-dashed border-primary/30 rounded-lg flex items-center justify-center pointer-events-none z-10">
            <div className="flex flex-col items-center gap-2 p-8">
              ...
              <p className="text-lg font-medium text-primary">Drop PDF file here to upload</p>
            </div>
          </div>
        )}
        ...
        <div className="border-t border-border p-4 bg-background">
          <div className="max-w-4xl mx-auto flex gap-3 items-end">
            <Textarea
              ...
              placeholder="Type your medical question... (မြန်မာ သို့မဟုတ် English)"
              className="flex-1 resize-none min-h-10 max-h-32 bg-card border-border transition-all duration-200 focus:border-primary/50 focus:ring-1 focus:ring-primary/20"
              rows={1}
            />
            <Button
              onClick={handleSend}
              ...
              size="icon"
              className="shrink-0 h-10 w-10 rounded-xl transition-all duration-200 active:scale-95"
            >
              {isLoading ? (
          ...
          <p className="text-xs text-muted-foreground/60 text-center mt-2 max-w-4xl mx-auto">
            Medicare Myanmar AI provides general medical information. Always consult a healthcare professional for medical advice.
          </p>
        </div>
      </div>
            ...
            {docs.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <Button
                  ...
                  size="sm"
                  className="mt-3"
                  onClick={() => { setShowDocsDialog(false); setShowUploadDialog(true); }}
                >
                  Upload Document
                </Button>
              </div>
